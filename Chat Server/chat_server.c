...
#include <arpa/inet.h>
  ...
  #define PORT 30000
#endif
#define MAX_BACKLOG 5 /*the maximum length to which the queue of pending connections for sockfd may grow*/
...
int accept_connection(int fd, struct sockname *usernames) {
    ...
    char user[BUF_SIZE];
    int num_read = read(client_fd, user, BUF_SIZE);
    user[num_read] = '\0';
    usernames[user_index].sock_fd = client_fd;
    usernames[user_index].username = malloc(num_read);
    strncpy(usernames[user_index].username, user, num_read);
    return client_fd;
}
...
int read_from(int client_index, struct sockname *usernames) {
    ...
    strcpy(buf, usernames[client_index].username);
    strncat(buf, ":", 2);
    int start_read = strlen(buf);
    int num_read = read(fd, &buf[start_read], BUF_SIZE);
    if(num_read  == 0){
      usernames[client_index].sock_fd = -1;
      return fd;
    }

    buf[num_read + start_read] = '\0';
